...
Interactive overview: [`docs/data-graph-overview.html`](docs/data-graph-overview.html) — open in a browser to explore nodes, edges, and implementation status. Cursor Canvas mirror: [`canvases/data-graph-overview.canvas.tsx`](canvases/data-graph-overview.canvas.tsx).
...
| **ExternalPerson**           | `external_person_id` slug (`ext:person:…`, `ext:role:…`, `ext:org:…`) | `just build-identity` (`external-identity` scan); bio via `enrich-external-persons` | Ministers, experts, procedural roles (Voorzitter), institutional authors. Separate from MPs; 95 entities in graph. |
...
| **AgendaItem**               | `{session}_{kind}_{meeting}_agenda_{start_block}`               | Meeting report headings (`h1`/`h2`, agenda numbers)                                                                                                                   | Stable id from timeline `start_block` (printed `agenda_id` is not unique). Staging `agenda_items.parquet`; dossier cites via `REFERENCES`. |
| **Utterance**                | `{session}_{kind}_{meeting}_{agenda}_{turn}` + `seq`            | Integraal verslag (plenary + commission); staging `utterances.parquet` (43 996 rows)                                                                                  | Full-session extraction: `question`, `general_debate`, `proposition`, `hearing`, `vote`, `notice`. Speakerless rows remain (chairs skipped, unresolved speakers). |
...
| **Dossier**                  | `{session_id}/{number}`                                         | [flwbn.cfm](https://www.dekamer.be/kvvcr/showpage.cfm?section=/flwb&language=nl&cfm=/site/wwwcfm/flwb/flwbn.cfm) and static `/flwb/html/{session}/N/{doc}.html` pages | 1,664 scraped via plenary refs plus full FLWB browse (`ListDocument.cfm` → `ListFromTo.cfm`).    |
| **Document**                 | FLWB doc id (`56K1280004`)                                      | Dossier subdocuments (PDF/HTML)                                                                                                                                       | 4,205 metadata rows, including each dossier's top-level `/001`; body text via PDF→markdown pipeline.                                                          |
...
| `PART_OF`           | Utterance → AgendaItem             | Containing agenda timeline slot                     | When `agenda_item_id` is set.                                  |
| `PART_OF`           | AgendaItem → Meeting               | Parent report                                       | Every agenda slot.                                             |
| `PART_OF`           | Question / Hearing / Interpellation → AgendaItem | Typed proceeding under its slot          | When `item_id` matches.                                        |
...
| `REFERENCES`        | Meeting → Dossier                  | Proposition/vote titles `(297/10)`                  | Prefer AgendaItem→Dossier; Meeting-level edge not required.    |
| `REFERENCES`        | AgendaItem → Dossier               | Agenda heading `(56/318)`                           | Working; inherited by utterances via PART_OF.                  |
...
| **Canonicalized and working**              | Identity: Person, ExternalPerson (95), Party, Commission, memberships, person/external aliases; normalized edges (`vote_casts`, `authored`, `asked`, `answered`, `interpellated`, `interpellation_responded`, `invited`, `holds_role`, `utterances`); graph Parquet (82 357 nodes / 378 488 edges); Hearing + Interpellation + Answer/QRVA nodes with `PART_OF` / `HAS_ANSWER` links |
...
| **Not scraped / not normalized**           | Motion (as node), InterventionAnalysis, MediaRecording, Meeting↔Dossier calendar (`DISCUSSED_IN`), Beknopt verslag, `DECLARES_INTEREST`, **commission roll-call votes** (not present in integraal verslag HTML) |
...
Verified against the current workspace on 2026-07-16. Counts below are from the last cache-backed full pipeline run (`build-identity` → `normalize-edges` → `build-graph`). Interactive overview: [`docs/data-graph-overview.html`](docs/data-graph-overview.html).
...
- **Step 1 identity (working):** `just build-identity` runs `identity` + `external-identity`. Writes `persons.parquet`, `parties.parquet`, `person_aliases.parquet`, `commissions.parquet`, `memberships.parquet`, `external_persons.parquet` (95), `external_person_aliases.parquet`, `external_person_contexts.parquet`, `unresolved_persons.parquet`, and `unresolved_report.md`. Last run: 175 persons, 13 parties.
- **Step 2 normalize (working):** `just normalize-edges` routes all person-bearing edges through `Resolver` or `ActorResolver` into `data/normalized/*.parquet` (including vote casts, authored, asked, answered, interpellated, utterances).
...
- **Dossier discovery (Step 4, done):** `just scrape-dossiers` unions plenary-derived ids with FLWB browse (`ListDocument.cfm` → `ListFromTo.cfm`). Graph now has 1 664 Dossier nodes and 4 205 source-linked Document nodes, including primary `/001` documents.
- **Full-session utterances (Step 4, done):** `crawl` report-block stream + agenda timeline + speaker segmentation writes staging `utterances.parquet` for plenary and commission (43 996 rows). Normalized and graphed as Utterance nodes with `SPOKE` / `PART_OF`.
...
- **External identity + ActorResolver (Step 1–2, done):** `external-identity` scans staging for non-MP actors; `ActorResolver` routes speakers, authors, and respondents to Person or ExternalPerson. Optional `enrich-external-persons` adds LLM bios.
- **Written Q&A / Answer nodes (Step 4, done):** QRVA routes and answers land as Question / Answer nodes with `ASKED`, `ADDRESSED_TO`, `HAS_ANSWER`, `ANSWERED_BY`, and optional `REFERENCES` (exact oral merge).
...
Government/minister mandate-over-time table (portfolio titles resolve to ExternalPerson today, not dated mandates) and remaining Step 4 sources (Motion node, dossier calendar edges, Beknopt verslag, INQO id enrichment). Stage 0 schema-hygiene checks are partially reimplemented in `just qa` (`schema.*` tier).
