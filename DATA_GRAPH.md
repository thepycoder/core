...
| **Utterance**                | `{session}_{kind}_{meeting}_{agenda}_{turn}` + `seq`            | Integraal verslag (plenary + commission); full-session speech rows in `utterances.parquet`                                                                            | One row per spoken turn; threads built at query time from `item_kind` / `item_id` / meeting filters.                    |
...
| **Canonicalized and working**              | Identity seed tables: Person, ExternalPerson, Party, Commission, party memberships, commission memberships, person/external aliases, and unresolved-person report; normalized edges (`vote_casts`, `authored`, `asked`, `answered`, `holds_role`, `utterances`) and graph Parquet (`nodes`, `edges`, `source_artifacts`) |
| **Parsed but not normalized**              | General plenary debate utterances (outside Q&A extraction), commission role edges beyond chair |
...
- **Remaining in this step:** minister/government resolution (portfolio-title→person-over-time table and `ANSWERED` edges). Utterance ids are stable per meeting report turn; `seq` is the thread order key within a meeting.
