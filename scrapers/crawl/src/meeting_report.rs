use crate::agenda_timeline::{build_agenda_timeline, MeetingKind};
use crate::report_blocks::{parse_report_blocks, read_report_html};
use crate::utterance_segment::{segment_utterances, UtteranceDraft};
use scraper::Html;
use std::error::Error;
use std::path::Path;
...
pub fn extract_utterances_from_document(
    ...
    let agenda = build_agenda_timeline(document, &blocks, meeting_kind, session_id, meeting_id);
    segment_utterances(
        &blocks,
        &agenda,
        meeting_kind,
