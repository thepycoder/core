...
fn check_meeting_gaps(data_dir: &Path) -> Result<Vec<CheckDetail>, Box<dyn Error>> {
    let path = data_dir
        .join(format!("sessions/{SESSION_ID}/commission/meeting_gaps.parquet"));
    if !path.exists() {
        return Ok(Vec::new());
    }
    let mut details = Vec::new();
...
fn check_cache_exists(data_dir: &Path) -> Result<Vec<CheckDetail>, Box<dyn Error>> {
    ...
    for rel in tables {
        ...
        for batch in read_all_rows(&path)? {
            if !batch.schema().fields().iter().any(|f| f.name() == "cache_path") {
                continue;
            }
            let cache_paths = read_string_column(&batch, "cache_path")?;
            let has_source_url = batch.schema().fields().iter().any(|f| f.name() == "source_url");
